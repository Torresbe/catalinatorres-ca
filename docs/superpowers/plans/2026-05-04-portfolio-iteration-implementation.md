...
Replace the contents of `src/lib/claude.ts` with the following (deletes `ClassifyResult`, `CLASSIFY_SYSTEM`, and `classifyText`; keeps `extractJSON`, all workflow types, and `suggestWorkflow`):
